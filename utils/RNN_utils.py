...
def train_RNN_merged(epochs, train_loader, train_loader_aug, valid_loader, valid_loader_aug, model, loss_fn, optimizer, eval_every=0.25, best_valid_loss=float("Inf"), device='cuda', model_save_name='', save_dir='./'):
    model.train()
    ...
    for epoch in tqdm(range(epochs)):
        ...
        t_correct = 0
        t_samples = 0
        for images, labels, notes in merged_loaders(train_loader, train_loader_aug):
            optimizer.zero_grad()
            ...
            if (global_step % (int(eval_every*len(train_loader.dataset)))) < train_loader.batch_size:
                ...
                with torch.no_grad():
                    ...
                    for images, labels, notes in merged_loaders(valid_loader, valid_loader_aug):
...
def evaluate_RNN(model, test_loader, test_loader_aug, device="cuda"):
    model.eval()
    y_pred = []
    y_true = []
    ...
    total_acc_test = 0
    with torch.no_grad():
        for images, labels, notes in merged_loaders(test_loader, test_loader_aug):
            coeffs = pywt.wavedec(images, wavelet, level=level, axis=1)
            threshold = 0.1 * \
                torch.median(torch.abs(torch.from_numpy(coeffs[-1])))
            denoised_coeffs = [pywt.threshold(
